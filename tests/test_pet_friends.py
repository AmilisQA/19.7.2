...
def test_add_new_pet_with_valid_data(name='Gir', animal_type='Giraffe',
...
def test_successful_update_self_pet_info(name='newGira', animal_type='Жираф', age=5):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "ФаФа", "Жирафа", "3", "images/0.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_get_api_key_for_invalid_user(email=invalid_email, password=valid_password):
    """Проверяем на статус ответа при вводе неверного Email """
    status = pf.get_api_key(email,password)
    assert status != 200

def test_get_api_key_for_invalid_user(email=valid_email, password=invalid_password):
    """Проверяем на статус ответа при вводе неверного пароля """
    status = pf.get_api_key(email,password)
    assert status != 200


def test_get_all_pets_with_valid_key_wrong_filter(filter='Жирафы'):
    """ При передаче в значение фильтра
...
def test_add_new_pet_with_large_name(name='!йцукаreeer&&&&)))))))))00000$544554hjknbmcvmmnvmvnnmnmcn'
                                     , animal_type='',age=''):
...
def test_add_new_pet_with_large_age(name=''
                                     , animal_type=''
                                     ,age='679689668989549949589000000000004043373737373773737377'):
...
def test_add_new_pet_without(name='', animal_type='',
                                     age=''):
...
def test_add_new_pet_incorrect(name='Жорик', animal_type='жирафик',
                                     age='семь'):
